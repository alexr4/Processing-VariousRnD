...
#define SURFACE_DIST 0.005 // minimum distance for a Hit
...
float getDist(vec3 p){
  float planeDist = p.y + 1.0; //get the distance from the ground
  ...
  float sdA = sdSphere(spA, 4.5);

	vec3 v = voronoi(2. * p + u_time * 0.5);

	float f = clamp(1.0*(v.y-v.x), 0.0, 1.0 );
	float addsdA = sdA - 0.25 * f;

  float lattice = opSubstract(addsdA, sdA);
  float final = opMorph(lattice, sdA, modTime);
...
vec3 render(vec3 ro, vec3 rd){
  ...
  if(d > 0.0){
    ...
    vec3 lightPos = vec3(5, 10, 5);//5, 10.0, 50.0);
    ...
    float diff = clamp(dot(nor, lig), 0.0, 1.0) *
                softShadowImproved(pos, lig, 25.0);
