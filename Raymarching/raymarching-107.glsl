...
void main(){
  ...
  vec3 grain = addGrain(uv, u_time, 0.05).xyz;
  float cols = 4.0;
  float diag = (ouv.x + ouv.y) * 0.5;
  float colsIndex = floor(diag * cols);
  vec3 buffers =
    depth * (1.0 - step(1.0, colsIndex)) +
    perf * (step(1.0, colsIndex) - step(2.0, colsIndex)) +
    normal * (step(2.0, colsIndex) - step(3.0, colsIndex)) +
    (color + grain) * step(3.0, colsIndex);

  gl_FragColor = vec4(buffers, 1.0);
}
