/**
To Do :
* Retake the full render method where 
* * Raymarching are outside the render function
* * Add params (dist, step.;;;;;;; ;..) as in variable for the raymarched function
* * DoF can be compute outside the Render function
* * Try to avoid recomputing variable surch as normal, pos, ro...
* * index need to be an inout variable in order to set relfection on specific objects
*/
#ifdef GL_ES
...
vec2 getDist(vec3 p){
  vec2 plane = sdPlane(p + 150, vec4(0, 1, 0, 1), 0.0);
  vec2 scene = plane;
...
float ambientOcclusion(vec3 p, vec3 n){
  ...
  for(int i=0; i<OCCSTEP; i++){
    float h = 0.001 + 0.15 * float(i)/float(OCCSTEP);
...
vec4 render(vec3 ro, vec3 rd, inout vec3 pos, inout vec3 nor){
  vec3 col = vec3(0.0);
  ...
  if(d.x > 0.0){
    ...
    pos = ro + rd * d.x;
    nor = getNormal(pos, 0.025);
    vec3 eye = normalize(ro);
    vec3 view = normalize(-rd);
    
    
    float isGround = (index <= 0.0) ? 1.0 : 0.0;
    //texturing
    ...
    vec3 mat = albedoMapping * (1.0 - isGround) + vec3(0.25) * isGround;
...
void main(){
  ...
  vec3 pos;
  vec3 nor;
  vec4 color = render(ro, rd, pos, nor);

  //recursion is not allowed in raymarching
  int reflCount = 1;
  if(reflCount > 0){
       rd = normalize(reflect(rd, nor));
       ro = pos + (rd * 0.01);
       vec4 refl = render(ro, rd, pos, nor);
       color.rgb += refl.rgb * 1.0;
   }
