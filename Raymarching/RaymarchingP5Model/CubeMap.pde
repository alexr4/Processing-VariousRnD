...
void generateCubeMap(String filepath) {
  ...
  /*From Nacho Cossio : 
  the problem comes from the cubemap. there is no processing functino to assign a texture slot, 
  so its hardcoded in the librery. When you use pass each texture to a shader via processing, 
  it auto assign the texture slot, so it might overwrite the cubemap but if you harcode the cubemap 
  to a very hight slot index, it might throw an error as smaller graphics cards might not have so many slots
  if you have a powerfull one, you might just use a higher index for the cubemap initialization
