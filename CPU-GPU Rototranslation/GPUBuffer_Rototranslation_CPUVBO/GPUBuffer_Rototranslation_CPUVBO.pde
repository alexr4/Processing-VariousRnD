...
This example show a full GPU rototranslation computation into fragement shader (buffer) using float packing
 and update of point cloud on cpu side
 key :
 p : play/pause
...
void draw() {
  ...
  float[] data = fp.decodeARGB24Float(buffer);
  int stride = imgW * imgH;
  float bounding = 2000.0;
  for (int i=0; i<data.length / 3; i++) {
    float x = data[i] * 2.0 - 1.0;
    float y = data[i + stride] * 2.0 - 1.0;
    float z = data[i + stride * 2] * 2.0 - 1.0;

    vbo.setVertex(i, x * bounding, y * bounding, z * bounding);
  }
  vbo.updateVBO();
  //exit();
...
void keyPressed() {
  ...
  if (key == 's')
    saveFrame("GPGPU_Rototransalation+CPUVBO.png");;
}
