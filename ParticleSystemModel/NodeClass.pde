...
public class PhysicBehaviour extends PhysicsBase {
  ...
  public void separate(ArrayList<NodeBase> nodes, float power, float desiredSeparation) {
    PVector force = getSeparate(nodes, desiredSeparation, false);
    force.mult(power);
    applyForce(force);
  }

  public void separate(ArrayList<NodeBase> nodes, float power, float desiredSeparation, boolean isMassRadius) {
...
public class PhysicsEdge extends PhysicsBase {
  ...
  public void bounce2D(AABB2D aabb) {
    bounce2D(aabb, 1.5);
  }

  public void bounce2D(AABB2D aabb, float friction) {
  ...
  public void infinite2D(AABB2D aabb) {
    if (node.location.x < aabb.x - aabb.w * 0.5) {
      node.location.x = aabb.x + aabb.w * 0.5;
    } else if (node.location.x > aabb.x + aabb.w * 0.5) {
      node.location.x = aabb.x - aabb.w * 0.5;
  ...
  public void warpArround2D(AABB2D aabb) {
    PVector desired = null;

    if (node.location.x > aabb.x + aabb.w * 0.5) {
      desired = new PVector(node.maxSpeed * -1.0, node.velocity.y);
    } else if (node.location.x < aabb.x - aabb.w * 0.5) {
      desired = new PVector(node.maxSpeed, node.velocity.y);
    }

    if (node.location.y > aabb.y + aabb.h * 0.5) {
      desired = new PVector(node.velocity.x, node.maxSpeed * -1.0);
    } else if (node.location.y < aabb.y - aabb.h * 0.5) {
      desired = new PVector(node.velocity.x, node.maxSpeed);
    }
  ...
  public void respawnAt(PVector respawn) {
    if (   node.location.x > aabb.x + aabb.w * 0.5
      || node.location.x < aabb.x - aabb.w * 0.5
      || node.location.y > aabb.y + aabb.h * 0.5
      || node.location.y < aabb.y - aabb.h * 0.5) {
  ...
  public void respawnAtRandom(AABB2D aabb) {
    if (   node.location.x > aabb.x + aabb.w * 0.5
      || node.location.x < aabb.x - aabb.w * 0.5
      || node.location.y > aabb.y + aabb.h * 0.5
      || node.location.y < aabb.y - aabb.h * 0.5) {

      node.location = new PVector(random(aabb.x - aabb.w * 0.5, aabb.x + aabb.w *0.5), random(aabb.y - aabb.h * 0.5, aabb.y + aabb.h *0.5));
    }
  }
  ...
  PVector reflect(PVector vel, PVector norm) {
    ...
    return new PVector(2*norm.x*dotDN - dir.x, 2*norm.y*dotDN - dir.y);
  }
}
...
public class PhysicsDebug extends PhysicsBase {
  ...
  public void arrow(PGraphics c, float len) {
    ...
    c.line(node.location.x, node.location.y, shaft.x, shaft.y);
    c.line(shaft.x, shaft.y, arrowRight.x, arrowRight.y);
    c.line(shaft.x, shaft.y, arrowLeft.x, arrowLeft.y);
  }
  ...
  public void display(PGraphics c) {
    c.ellipse(node.location.x, node.location.y, node.mass, node.mass);
  }
