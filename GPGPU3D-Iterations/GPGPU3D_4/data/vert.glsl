...
void main() {
  ...
  vec4 rgbaX = texture(posImage, uv3d.uvx);
  vec4 rgbaY = texture(posImage, uv3d.uvy);
  vec4 rgbaZ = texture(posImage, uv3d.uvz);

  float nx = decodeRGBA24(rgbaX.rgb);
  float ny = decodeRGBA24(rgbaY.rgb);
  float nz = decodeRGBA24(rgbaZ.rgb);
