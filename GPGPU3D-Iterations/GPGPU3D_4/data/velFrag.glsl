...
void main(){
	...
	interleavedUVS iuvs = getInterleavedXYZUV(uv, pindex, textureResolution);
  	...
  	vec3 nvel = getData(iuvs, texture, isXYZ);
  	vec3 nloc = getData(iuvs, posBuffer, isXYZ);
    ...
    */
	...
	// vec3 friction = normalize(vel * -1.0) * coeff;
  	...
  	vel /= maxSpeed;
  	vel = vel * 0.5 + 0.5; //remap from -1, 1.0 to 0.0, 1.0
  	vel = clamp(vel, 0.0, 1.0);// clamp value (security only)

  	vec4 newVel = encodeRGBA24(vel.x) * isXYZ.x +
  				  encodeRGBA24(vel.y) * isXYZ.y +
  				  encodeRGBA24(vel.z) * isXYZ.z;
