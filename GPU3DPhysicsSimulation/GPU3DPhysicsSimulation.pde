...
void init(int w, int h) {
  ...
  //PShape obj = loadShape("Einstein.obj");
  //int[] wh = GPUImage.getWidthHeightFromArea(obj.getChildCount());
  //WIDTH = wh[0];
  //HEIGHT = wh[1];

  nbElement = (WIDTH * HEIGHT);
  originalData =  getPosData(WIDTH, HEIGHT);//loadObjAsArray(obj, 50);//getPosData(w, h);
