...
vec3 getData(vec2 screenPosition, vec2 gridResolution, vec2 samplerResolution, sampler2D dataSampler){
	...
	float ux = round(mod(i0, samplerResolution.x));
	float vx = round((i0 - ux) / samplerResolution.x);

	float uy = round(mod(i1, samplerResolution.x));
	float vy = round((i1 - uy) / samplerResolution.x);

	float uz = round(mod(i2, samplerResolution.x));
	float vz = round((i2 - uz) / samplerResolution.x);
