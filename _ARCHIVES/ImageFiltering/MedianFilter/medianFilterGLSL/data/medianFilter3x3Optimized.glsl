...
void main(){
	...
	pix[0] = texture(texture, invuv + vec2(-1.0, -1.0) * uvinc).rgb;
	pix[1] = texture(texture, invuv + vec2(0.0, -1.0)  * uvinc).rgb;
	pix[2] = texture(texture, invuv + vec2(1.0, -1.0)  * uvinc).rgb;
	pix[3] = texture(texture, invuv + vec2(-1.0, 0.0)  * uvinc).rgb;
	pix[4] = texture(texture, invuv + vec2(.0, .0)     * uvinc).rgb;
	pix[5] = texture(texture, invuv + vec2(1.0, .0)    * uvinc).rgb;
	pix[6] = texture(texture, invuv + vec2(-1.0, 1.0)  * uvinc).rgb;
	pix[7] = texture(texture, invuv + vec2(.0, 1.0)    * uvinc).rgb;
